...
const Services = () => {
    ...
    const getSegmentPoints = useCallback((startAngle: number, endAngle: number) => {
    ...
    const getIconPoint = useCallback((angle: number) => getPoint(angle, innerRadius + (radius - innerRadius) / 2), [getPoint]);
    const getLineEndPoint = useCallback((angle: number) => getPoint(angle, radius + 20), [getPoint]);

    const getHoverTranslate = useCallback((angle: number, amount: number) => {
        const rad = degToRad(angle - 90);
        return { tx: Math.cos(rad) * amount, ty: Math.sin(rad) * amount };
    }, [degToRad]);

    const getTextProps = useCallback((angle: number) => {
    ...
    const innerHexPoints = useMemo(() => Array.from({ length: 6 }).map((_, i) => {
                ...
                <div className="md:hidden grid grid-cols-2 gap-3">
                    {servicesData.map((service, i) => (
                        ...
                        >
                            ...
                            <div className="w-12 h-12 rounded-xl bg-renk1 flex items-center justify-center mb-3
                                            group-hover:scale-110 transition-transform duration-300"
                                style={{ clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)' }}
                            >
                                {React.cloneElement(service.icon as React.ReactElement, { size: 22, className: "text-white" } as any)}
                            </div>
                            <h3 className="text-sm font-bold text-renk1 leading-snug">{service.title}</h3>
                            {/* Alt dekoratif çizgi */}
                            <div className="absolute bottom-0 left-4 right-4 h-[2px] bg-gradient-to-r from-transparent via-renk2/40 to-transparent 
